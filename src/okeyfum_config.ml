...
module Keydef_map = Map.Make (struct
  ...
  let compare (aa, ab) (ba, bb) =
    let key_compared = Stdlib.compare aa ba in
    if key_compared <> 0 then key_compared else match (ab, bb) with `UP, `DOWN -> -1 | `DOWN, `UP -> 1 | _ -> 0
end)
...
let parse_with_error lexbuf =
  try Config_parser.parser_main Config_lexer.token lexbuf
  with Config_parser.Error ->
...
module Config : sig
  ...
  let stmt_to_sequence map = function
    ...
    | Config_type.Cstm_default (state, seqs) -> Keydef_map.add (wildcard_key, state) (List.map exp_to_key seqs) map
  ...
  let stmt_to_decl = function
    | Config_type.Cstm_lock (lock_names, seqs) ->
        ...
        let lock_names = List.map (function Config_type.Cexp_ident name -> Some name | _ -> None) lock_names in
  ...
  let stmt_to_lockdef = function
    | Config_type.Cstm_deflock (Config_type.Cexp_ident lock_name) -> lock_name
  ...
  let rec build_statement config stmts =
    ...
    | stmt :: rest -> (
        match stmt with
        | Config_type.Cstm_lock _ ->
            build_statement { config with lock_decls = stmt_to_decl stmt :: config.lock_decls } rest
  ...
  let empty = { variable_map = Hashtbl.create 100; lock_decls = []; lock_defs = []; keydef_map = Keydef_map.empty }

  let make = function Config_type.Cprog_main stmts -> build_statement empty stmts
end
