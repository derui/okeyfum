...
let release_all_keys fd =
  ...
  let input = { U.time = Time.now (); typ = T.Event_type.ev_key; value = 0L; code = 0 } in
  ...
  let data =
    U.(
      of_ocaml { input with U.typ = T.Event_type.ev_syn; time = Time.now (); code = T.Input_syn.report }
      |> addr |> to_voidp)
  in
  Inner.dev_write fd data (sizeof U.t |> Unsigned.Size_t.of_int) |> ignore
...
let open_user_dev () =
  ...
  let fd =
    List.fold_left (fun fd path -> if fd >= 0 then fd else Inner.dev_open path T.Open_flag.wronly) (-1) uinput_paths
  in
  if fd < 0 then failwith "Not found any uinput device" else Log.debug "Opening uinput device finished";
  fd
...
let create_user_dev fd =
  ...
  let udev =
    {
      U.name = "okeyfum";
      id = { Types.Input_id.bustype = T.Input_bus.bus_virtual; vendor = 1; product = 1; version = 1 };
      ff_effects_max = 0;
      absmax = [];
      absmin = [];
      absfuzz = [];
  ...
  List.iter
    (fun _ -> Inner.ioctl fd Signed.Int64.(of_int UI.ui_set_evbit) (Util.int_to_voidp T.Event_type.ev_key) |> ignore)
    [ T.Event_type.ev_key; T.Event_type.ev_rep ];

  (* initialize keycodes raise from created device *)
  let keys = Array.make 256 0 in
  Array.iteri
    (fun ind _ -> Inner.ioctl fd Signed.Int64.(of_int UI.ui_set_keybit) (Util.int_to_voidp ind) |> ignore)
    keys;
...
let close_key_dev ?(grab = true) fd =
  ...
  if grab then Inner.ioctl fd Signed.Int64.(of_int T.Input_ioctl.grab) (Util.int_to_voidp 0) |> ignore else ();
...
let open_with ?(grab = true) ~dev ~f () =
  let fd : keyboard fd ref = ref (-1) and ufd : user fd ref = ref (-1) in
  ...
  with Unix.Unix_error (e, fname, param) ->
...
let read_key fd =
  ...
  Log.debug
    (Printf.sprintf "read key type = %d; code = %d; value = %Ld; len = %d" data.U.typ data.U.code data.U.value
...
let write_key fd data =
  let module U = Types.Input_event in
  let module UI = T.Uinput in
  Log.debug (Printf.sprintf "write key type = %d; code = %d; value = %Ld" data.U.typ data.U.code data.U.value);
