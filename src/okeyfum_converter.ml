...
let rec expand_key config state = function
  | `Id k                 -> (
      ...
      | Some c ->
          let k_d = { IE.code = c; time = T.Timeval.empty; value = 1L; typ = GT.Event_type.ev_key } in
      ...
      | None   ->
          ...
          raise Not_found )
  | `Var k                ->
      let vars = C.variable_map config in
...
let is_not_key_event { IE.typ; _ } = typ <> GT.Event_type.ev_key
...
let expand_event config env event =
  ...
  let key_map =
    match E.locked_keys env with
    | []   -> Some (C.keydef_map config)
    | keys -> (
        let module S = Okeyfum_config.Lock_set in
        ...
        match decls with [] -> None | (_, map) :: _ -> Some map )
  in
...
let convert_event_to_seq ~config ~env ~event =
  ...
  else match expand_event config env event with None -> T.No_def default_seq | Some seq -> T.To_eval seq
